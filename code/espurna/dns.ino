...
char _dns_hostname[64];
ip_addr_t _dns_ipaddr;
...
class dns_resolver_cb_t {
public:
    dns_resolver_cb_t(dns_resolver_cb_f cb) : _callback(cb) {}
    ~dns_resolver_cb_t() {}
    void operator()(const char *arg) {
        _callback(arg);
    }
protected:
    dns_resolver_cb_f _callback = nullptr;
};
...
void _dns_found_callback(const char *name, const ip_addr_t *ipaddr, void *arg) {
    ...
    ip_addr_set(&_dns_ipaddr, ipaddr);

    if (arg) {
        auto *cb = static_cast<dns_resolver_cb_t*>(arg);
        (*cb)(ipaddr_ntoa(&_dns_ipaddr));
        delete cb;
    }
}

void _dns_gethostbyname(const char* name, dns_resolver_cb_f callback = nullptr) {

    if (strlen(name) > (sizeof(_dns_hostname) - 1)) {
        DEBUG_MSG_P(PSTR("Hostname is too long!\n"));
        return;
    }
    ...
    if (callback) {
        dns_resolver_cb_t *_dns_cb = new dns_resolver_cb_t(callback);
        error = dns_gethostbyname(
                name, &result, (dns_found_callback)&_dns_found_callback, (void*)_dns_cb);
    } else {
    ...
    if (error == ERR_OK) {
        _dns_state = dns_state_t::DONE;
        ip_addr_set(&_dns_ipaddr, &result);
        return;
...
void _dnsLoop() {
    switch (_dns_state) {
        ...
        case (dns_state_t::DONE):
            _dns_state = dns_state_t::IDLE;
            const char* res = ipaddr_ntoa(&_dns_ipaddr);
            DEBUG_MSG_P(PSTR("[DNS] Found IP for %s: %s\n"), _dns_hostname, res);
            return;
    }
}

void dnsResolve(const char* hostname, dns_resolver_cb_f callback) {
    ...
    _dns_gethostbyname(hostname, callback);
}
