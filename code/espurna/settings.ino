...
bool settingsRestoreJson(const JsonObject& data) {
    ...
    for (auto element : data) {
        if (strcmp(element.key().c_str(), "app") == 0) continue;
        if (strcmp(element.key().c_str(), "version") == 0) continue;
        if (strcmp(element.key().c_str(), "backup") == 0) continue;
        setSetting(element.key().c_str(), element.value().as<char*>());
    }
...
bool settingsRestoreJson(char* json_string, size_t json_buffer_size = 1024) {
    ...
    if (error) {
        DEBUG_MSG_P("[SETTINGS] JSON parsing error: %s\n", error.c_str());
        return false;
    }
